  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: `Predplatné — ${BRAND.name}` },
      {
        ...
        content: "Plán, kvóty a správa fakturácie.",
      },
    ],
  }),
...
function SubscriptionScreen() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      fetchState({ data: { environment: getStripeEnvironment() } }),
    enabled: configured,
  });
    ...
    <PhoneFrame>
      ...
      <Screen>
        ...
        {!configured ? (
          <Card>
            <p className="text-sm font-semibold">
              Platby nie sú nakonfigurované
            </p>
            <p className="mt-1 text-[11px] text-muted-foreground">
              Aplikácia funguje bez platieb. Platený plán bude dostupný po
              dokončení nastavenia platieb v projekte — dovtedy sa tu
              nezobrazuje žiadna úspešná platba.
            </p>
          </Card>
        ) : null}
        ...
        <Card className="space-y-2">
          ...
          <p className="text-[11px] text-muted-foreground">
            Aj po skončení predplatného zostáva prístup k existujúcim údajom,
            ich exportu aj vymazaniu. Obmedzia sa len nové serverové operácie
            nad rámec základných kvót.
          </p>
        </Card>
        ...
        {plan === "pro" || data?.status !== "inactive" ? (
          <Button
            ...
            disabled={!configured}
          >
            <ExternalLink className="mr-1 h-4 w-4" aria-hidden /> Správa
            fakturácie
          </Button>
        ) : null}

        <p className="text-[10px] text-muted-foreground">
          Ceny sú testovacie a nepredstavujú finálny cenník. Stav predplatného
          sa vždy overuje u poskytovateľa platby, nie podľa návratu z platobnej
          stránky.
        </p>
      </Screen>
      <BottomNav />
